...
import { ConflictError, NotFoundError, ConcurrencyError } from '../../../src/domains/types';
...
describe('Business Domain', () => {
  ...
  describe('findByEmail', () => {
    it('should find business by email', async () => {
      ...
      const found = await BusinessService.findByEmail('unique@business-test-domain.com');
      expect(found).toBeTruthy();
      expect(found?.email).toBe('unique@business-test-domain.com');
    });

    it('should return null if email not found', async () => {
      const found = await BusinessService.findByEmail('nonexistent@business-test-domain.com');
      expect(found).toBeNull();
    });
  });
  ...
  describe('listActive', () => {
    it('should list only active businesses', async () => {
      ...
      const active = await BusinessService.create({
      ...
      const toDeactivate = await BusinessService.create({
        email: 'inactive@business-test-domain.com',
        name: 'Inactive Business',
        reward_structure: { stamps_required: 10, reward_description: 'Reward' },
      });
      await BusinessService.deactivate(toDeactivate.id);

      const activeList = await BusinessService.listActive();
    ...
    it('should return empty array if no active businesses', async () => {
      const list = await BusinessService.listActive();
  ...
  describe('update', () => {
    ...
    it('should use optimistic locking on update', async () => {
      ...
      await BusinessService.update(business.id, { name: 'First Update' }, business.version);

      // This should fail because version changed
      await expect(
        BusinessService.update(business.id, { name: 'Second Update' }, business.version)
  ...
  describe('deactivate', () => {
    it('should deactivate business', async () => {
      ...
      const found = await BusinessService.findById(business.id);
      expect(found?.is_active).toBe(false);
    });

    it('should throw NotFoundError if business does not exist', async () => {
      await expect(
        BusinessService.deactivate('00000000-0000-0000-0000-000000000000')
  ...
  describe('reactivate', () => {
    it('should reactivate deactivated business', async () => {
      ...
      await BusinessService.deactivate(business.id);
      await BusinessService.reactivate(business.id);
  ...
  describe('count', () => {
    it('should count all businesses', async () => {
      const countBefore = await BusinessService.count();
      ...
      const countAfter = await BusinessService.count();
      expect(countAfter - countBefore).toBe(2);
    });

    it('should count only active businesses', async () => {
      const activeCountBefore = await BusinessService.count(true);
      ...
      const inactive = await BusinessService.create({
        email: 'inactive2@business-test-domain.com',
        name: 'Inactive',
        reward_structure: { stamps_required: 10, reward_description: 'Reward' },
      });
      await BusinessService.deactivate(inactive.id);

      const activeCountAfter = await BusinessService.count(true);
